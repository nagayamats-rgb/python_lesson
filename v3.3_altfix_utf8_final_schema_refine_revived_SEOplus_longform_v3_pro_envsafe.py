# -*- coding: utf-8 -*-
"""
ALT長文生成 v3_pro_envsafe
- .envの指定モデル・温度・トークンを動的に利用
- "chat" / "responses" モードを自動切替
- その他ロジックは v3_pro と同一
"""
...
def summarize_knowledge():
    ...
    kb = f"知見: 以下の語を自然に含めてALTを生成。推奨語句: {joined_terms}。1〜2文構成で句点で終える自然文を作成。"
...
def call_openai_20_lines(client, model, product, kb_text, forbidden, retry=3, wait=5):
    ...
    mode = os.getenv("OPENAI_MODE", "chat")
    temperature = float(os.getenv("OPENAI_TEMPERATURE", 1.3))
...
def main():
    print("🌸 ALT長文生成 v3_pro_envsafe（.envモデル対応）")
