# -*- coding: utf-8 -*-
"""
ALT長文生成 v4.5（自然文強制＋羅列検知自動リライト）
------------------------------------------------------------
・名詞の羅列を禁止、1〜2文の自然文で必ず句点終止
・AI生成後に「自然文チェック＆自動リライト」ローカル処理を追加
・禁則語除去、句点補完、重複除去、長さ制御を統合
・入力: ./rakuten.csv（UTF-8, ヘッダに「商品名」）
・出力:
    - output/ai_writer/alt_text_ai_raw_longform_v4.5.csv
    - output/ai_writer/alt_text_refined_final_longform_v4.5.csv
    - output/ai_writer/alt_text_diff_longform_v4.5.csv
"""
...
RAW_PATH = os.path.join(OUT_DIR, "alt_text_ai_raw_longform_v4.5.csv")
REF_PATH = os.path.join(OUT_DIR, "alt_text_refined_final_longform_v4.5.csv")
DIFF_PATH = os.path.join(OUT_DIR, "alt_text_diff_longform_v4.5.csv")
...
def init_env_and_client():
    ...
    temp = float(os.getenv("OPENAI_TEMPERATURE", "1").strip())
...
def load_products(path):
    ...
    with open(path, "r", encoding="utf-8", newline="") as f:
        ...
        if "商品名" not in r.fieldnames:
            raise SystemExit("CSVに『商品名』列がありません。")
        for row in r:
...
def summarize_knowledge():
    """
    ./output/semantics/ 内の JSON/CSV 群をざっくり要約テキスト化。
    list/dict 混在にも対応（全ファイルで安全動作）。
    """
    ...
    for p in files:
        ...
        if "lexical" in name or "cluster" in name:
            if isinstance(data, dict):
                arr = data.get("clusters") or []
            elif isinstance(data, list):
                arr = data
            else:
                arr = []
            for c in arr:
                if isinstance(c, dict) and "terms" in c:
        ...
        if "semantic" in name:
            if isinstance(data, dict):
                ...
                concept += [v for v in data if isinstance(v, str)]

        # --- template ---
        if "template" in name:
            if isinstance(data, dict):
                tmpl += [v for v in data.get("templates", []) if isinstance(v, str)]
                tmpl += [v for v in data.get("hints", []) if isinstance(v, str)]
            elif isinstance(data, list):
                tmpl += [v for v in data if isinstance(v, str)]

        # --- forbidden words ---
        if "forbid" in name or "normalized" in name:
            if isinstance(data, dict):
                forbid_local += [v for v in data.get("forbidden_words", []) if isinstance(v, str)]
            elif isinstance(data, list):
                forbid_local += [v for v in data if isinstance(v, str)]
...
SYSTEM_PROMPT = (
    ...
    "必ず1〜2文の自然文で、句点「。」で終えること。\n"
    "名詞の羅列は禁止（例: 『iPhone14、iPhone15、充電、ケーブル』は禁止）。"
    "箇条書きや番号も禁止。\n"
    "文末は『〜できる』『〜です』『〜します』『〜に便利です』など動詞または丁寧語で終える。\n"
    "画像や写真、レビュー、ランキング、当店などのメタ語は使わない。\n"
    "良い例:\n"
    "『耐久性のある編み込みケーブルで、絡まりにくく長く使えます。』\n"
    "『MagSafe対応でiPhoneをしっかり固定し、デスク作業中も快適に充電できます。』\n"
)
...
# ====== OpenAI呼び出し ======
def call_openai_20_lines(client, model, temp, max_tokens, product, kb_text, forbid_words):
...
# ====== ローカル自然化 ======
RE_ENUM_HEAD = re.compile(r"^\s*[\d①②③④⑤⑥⑦⑧⑨⑩\-\*\・●]\s*[\.．、]?\s*")
RE_MANY_COMMAs = re.compile(r"、.*、.*、")
RE_VERBISH_END = re.compile(r"(です|ます|できます|しました|する|できる|になります|に最適|向け|備える|採用|保護|改善|便利|快適|解消)」?。?$")

def looks_listy(s):
    if not s: return True
    t = s.strip()
    if RE_ENUM_HEAD.search(t): return True
    if "。" not in t and RE_MANY_COMMAs.search(t): return True
    if not RE_VERBISH_END.search(t): return True
    return False

def rewrite_listy_to_sentence(s):
    t = RE_ENUM_HEAD.sub("", s).strip("・-—●　")
    bits = [b.strip(" 。、・-—") for b in t.split("、") if len(b.strip()) > 1]
    if not bits: return s.strip() + "。"
    head = "、".join(bits[:2])
    tail = "、".join(bits[2:]) if len(bits) > 2 else ""
    cand = f"{head}を備え、{tail}でも使いやすい設計です。" if tail else f"{head}に対応し、日常の利便性を高めます。"
    for ng in FORBIDDEN:
...
def refine_20_lines(raw_lines):
    ...
    while len(uniq) < 20:
        uniq.append(f"{uniq[-1]}より快適になります。")
    return uniq[:20]
...
def main():
    ...
    for p in tqdm(products, desc="🧠 生成中", total=len(products)):
        try:
            ...
            raw = [f"{p}は使いやすさと耐久性を両立し、快適に使用できます。"] * 20
