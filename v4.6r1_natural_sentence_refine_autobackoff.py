# -*- coding: utf-8 -*-
"""
ALT長文生成 v4.6r1（自然文＋安定生成＋自動バックオフ）
------------------------------------------------------------
- 対象: 楽天 ALT 専用
- 入力: ./rakuten.csv （UTF-8, ヘッダに「商品名」）
- 出力:
    1. output/ai_writer/alt_text_ai_raw_longform_v4.6r1.csv
    2. output/ai_writer/alt_text_refined_final_longform_v4.6r1.csv
    3. output/ai_writer/alt_text_diff_longform_v4.6r1.csv
- 機能:
    ✅ gpt-5対応（.env固定）
    ✅ 名詞羅列禁止・自然文強制
    ✅ OpenAI 429/Quota制限で10分自動バックオフ
    ✅ 空応答補完＋句点終止保証
"""
...
RAW_PATH = os.path.join(OUT_DIR, "alt_text_ai_raw_longform_v4.6r1.csv")
REF_PATH = os.path.join(OUT_DIR, "alt_text_refined_final_longform_v4.6r1.csv")
DIFF_PATH = os.path.join(OUT_DIR, "alt_text_diff_longform_v4.6r1.csv")
...
SYSTEM_PROMPT = (
    ...
    "名詞の羅列は禁止。文末は『〜できる』『〜です』『〜します』『〜に便利です』など丁寧語で終える。\n"
...
# ====== OpenAI呼び出し ======
def call_openai_20_lines(client, model, temp, max_tokens, product, kb_text, forbid_words, retry=2, wait=10):
    ...
    for attempt in range(retry):
        try:
            ...
            if "insufficient_quota" in str(e):
                print("💤 クォータ制限による一時停止。10分待機後に再試行します。")
...
# ====== メイン ======
def main():
    print("🌸 ALT長文生成 v4.6r1（自然文＋自動バックオフ）")
