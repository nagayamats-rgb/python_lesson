...
MODEL = "gpt-4o-mini"   # GPT-5非対応時の安定モデル
MAX_TOKENS = 800
ALT_COUNT = 20
...
def ai_generate_alt(product_name: str) -> list[str]:
    ...
    prompt = (
        ...
        f"各ALTは全角100〜130文字を目安に自然な日本語で書いてください。\n"
    ...
    for attempt in range(3):  # 最大3回再試行
        try:
            ...
            time.sleep(3)
    return [""] * ALT_COUNT  # 全失敗時

# --- メイン処理 ---
def main():
    print("🌸 writer_splitter_perfect_v3.3_altfix 実行開始（ALT長文安定化）")
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    with open(INPUT_FILE, newline="", encoding="cp932") as f:
